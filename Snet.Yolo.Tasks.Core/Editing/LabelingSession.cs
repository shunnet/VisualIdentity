...
public sealed class LabelingSession
{
    ...
    public ResultRow AddPolygon(IReadOnlyList<double> pointsX, IReadOnlyList<double> pointsY, string? label)
    {
        ...
        for (var index = 0; index < pointsX.Count; index++)
        {
            points.Add(new JsonArray(
                PercentMath.PixelsToPercent(pointsX[index], OriginalWidth!.Value),
                PercentMath.PixelsToPercent(pointsY[index], OriginalHeight!.Value)));
        }
    ...
    public ResultRow AddKeyPoint(double x, double y, string? label)
    {
        ...
        var value = row.Value!;
        ValueAccess.SetDouble(value, "x", PercentMath.PixelsToPercent(x, OriginalWidth!.Value));
        ValueAccess.SetDouble(value, "y", PercentMath.PixelsToPercent(y, OriginalHeight!.Value));
    ...
    public ResultRow AddEllipse(double centerX, double centerY, double radiusX, double radiusY, string? label)
    {
        ...
        var value = row.Value!;
        ValueAccess.SetDouble(value, "x", PercentMath.PixelsToPercent(centerX, OriginalWidth!.Value));
        ValueAccess.SetDouble(value, "y", PercentMath.PixelsToPercent(centerY, OriginalHeight!.Value));
        ValueAccess.SetDouble(value, "radiusX", PercentMath.PixelsToPercent(radiusX, OriginalWidth!.Value));
        ValueAccess.SetDouble(value, "radiusY", PercentMath.PixelsToPercent(radiusY, OriginalHeight!.Value));
    ...
    public void ResizeShape(string regionId, double x, double y, double width, double height)
    {
        ...
        switch (row.Type)
        {
            case RegionType.RectangleLabels:
                ApplyPixelRectToRow(row, new PixelRect(x, y, width, height));
                break;
        }
    }
...
public void MovePolygonVertex(string regionId, int index, double x, double y)
    ...
    public void MoveShape(string regionId, double deltaX, double deltaY)
    {
        ...
        switch (row.Type)
        {
            ...
            case RegionType.PolygonLabels:
                ...
                {
                    points.Add(new JsonArray(
                        PercentMath.PixelsToPercent(Clamp(px[index] + deltaX, 0, OriginalWidth!.Value), OriginalWidth!.Value),
                        PercentMath.PixelsToPercent(Clamp(py[index] + deltaY, 0, OriginalHeight!.Value), OriginalHeight!.Value)));
                }
            ...
            case RegionType.EllipseLabels:
                ...
                ValueAccess.SetDouble(value, "x", PercentMath.PixelsToPercent(Clamp(ex + deltaX, 0, OriginalWidth!.Value), OriginalWidth!.Value));
                ValueAccess.SetDouble(value, "y", PercentMath.PixelsToPercent(Clamp(ey + deltaY, 0, OriginalHeight!.Value), OriginalHeight!.Value));
    ...
    public ResultRow AddBrushMask(IReadOnlyList<double> pointsX, IReadOnlyList<double> pointsY, double size, string? label)
    {
        ...
        for (var i = 0; i < pointsX.Count; i++)
        {
            PaintCircle(mask, width, height, pointsX[i], pointsY[i], radius);
            if (i > 0) { PaintSegment(mask, width, height, pointsX[i - 1], pointsY[i - 1], pointsX[i], pointsY[i], radius); }
        }

        var value = row.Value!;
        value["format"] = "rle";
        value["rle"] = JsonArrayOf(RleCodec.Encode(mask).Select(b => (int)b));
        value["size"] = size;
        value["pointxs"] = JsonArrayOf(pointsX.Select(p => (double)p));
        value["pointys"] = JsonArrayOf(pointsY.Select(p => (double)p));
        ValueAccess.SetStringList(value, "brushlabels", label is null ? Array.Empty<string>() : new[] { label });
        return Commit(row);
    }
