...
namespace Snet.Yolo.Api.Controllers
{
    ...
    public class OperateController : OperateBaseController
    {
        ...
        /// </returns>
        [HttpPost]
        public async Task<OperateResult> IdentityAsync(int onnxIndex, [AllowedFileType(new[] { ".jpg", ".jpeg", ".png", ".bmp" })] IFormFile file, string paramJson, YoloDotNet.ExecutionProvider.OpenVino.OpenVino? openVino = null)
        {
            ...
            if (result.GetDetails(out List<OnnxData>? datas))
            {
                ...
                IdentityOperate operate = IdentityOperate.Instance(new IdentityData
                {
                    ...
                    Hardware = new OpenVinoExecutionProvider(Path.Combine(onnxData.path, onnxData.name), openVino),
        ...
        /// </returns>
        [HttpPost]
        public async Task<OperateResult> IdentityDrawAsync(int onnxIndex, [AllowedFileType(new[] { ".jpg", ".jpeg", ".png", ".bmp" })] IFormFile file, string paramJson, YoloDotNet.ExecutionProvider.OpenVino.OpenVino? openVino = null)
        {
            OperateResult result = await QueryAsync(onnxIndex);
            if (result.GetDetails(out List<OnnxData>? datas))
            {
