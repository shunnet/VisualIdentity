...
namespace Snet.Yolo.Api.Controllers
{
    ...
    public class OperateController : OperateBaseController
    {
        ...
        /// <inheritdoc/>
        public override string Tag => "OpenVino";
        ...
        /// <param name="openVino">OpenVino 数据，可为空</param>
        /// <returns>
        /// 识别结果<br/>
        /// 返回识别到的坐标数据
        /// </returns>
        [HttpPost]
        public Task<OperateResult> IdentityAsync(int onnxIndex, [AllowedFileType(new[] { ".jpg", ".jpeg", ".png", ".bmp" })] IFormFile file, string paramJson, YoloDotNet.ExecutionProvider.OpenVino.OpenVino? openVino = null)
            => IdentityCoreAsync(onnxIndex, file, paramJson, path => new OpenVinoExecutionProvider(path, openVino));
        ...
        /// </returns>
        [HttpPost]
        public Task<OperateResult> IdentityDrawAsync(int onnxIndex, [AllowedFileType(new[] { ".jpg", ".jpeg", ".png", ".bmp" })] IFormFile file, string paramJson, YoloDotNet.ExecutionProvider.OpenVino.OpenVino? openVino = null)
            => IdentityDrawCoreAsync(onnxIndex, file, paramJson, path => new OpenVinoExecutionProvider(path, openVino));
    }
}
