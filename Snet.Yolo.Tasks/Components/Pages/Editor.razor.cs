...
public partial class Editor : ComponentBase, IAsyncDisposable
{
    ...
    private void DeleteTextSpan(string id) { _textRows.RemoveAll(row => row.Id == id); StateHasChanged(); }
    ...
    private void DeleteAudioRow(string id) { _audioRows.RemoveAll(row => row.Id == id); if (_selectedAudioId == id) { _selectedAudioId = null; } StateHasChanged(); }
