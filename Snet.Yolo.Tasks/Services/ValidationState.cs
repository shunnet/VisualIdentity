...
/// <summary>与指定模型关联的验证图片及其最后一次识别结果。</summary>
...
public sealed class ValidationState
{
    ...
    /// <summary>把图片加入指定模型的队列，并将其设为当前图片。</summary>
    public ValidationImageState AddImage(string userName, int modelIndex, string name, string url)
    {
        lock (_gate)
        {
            var model = GetOrCreateModel(userName, modelIndex);
            var image = new MutableImage(Guid.NewGuid(), name, url);
    ...
    private static ValidationImageState Snapshot(MutableImage image)
        => new(image.Id, image.Name, image.Url, image.ResultJson, image.Detections.ToArray());
    ...
    private sealed class MutableImage(Guid id, string name, string url)
    {
