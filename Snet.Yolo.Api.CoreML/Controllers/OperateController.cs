...
namespace Snet.Yolo.Api.Controllers
{
    ...
    public class OperateController : OperateBaseController
    {
        ...
        /// <inheritdoc/>
        public override string Tag => "CoreML";
        ...
        /// <param name="adaptive">自适应 默认 true</param>
        /// <returns>
        /// 识别结果<br/>
        /// 返回识别到的坐标数据
        /// </returns>
        [HttpPost]
        public Task<OperateResult> IdentityAsync(int onnxIndex, [AllowedFileType(new[] { ".jpg", ".jpeg", ".png", ".bmp" })] IFormFile file, string paramJson, bool adaptive = true)
            => IdentityCoreAsync(onnxIndex, file, paramJson, path => new CoreMLExecutionProvider(path, adaptive));
        ...
        /// </returns>
        [HttpPost]
        public Task<OperateResult> IdentityDrawAsync(int onnxIndex, [AllowedFileType(new[] { ".jpg", ".jpeg", ".png", ".bmp" })] IFormFile file, string paramJson, bool adaptive = true)
            => IdentityDrawCoreAsync(onnxIndex, file, paramJson, path => new CoreMLExecutionProvider(path, adaptive));
    }
}
