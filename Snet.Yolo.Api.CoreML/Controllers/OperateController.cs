...
namespace Snet.Yolo.Api.Controllers
{
    ...
    public class OperateController : OperateBaseController
    {
        ...
        /// </returns>
        [HttpPost]
        public async Task<OperateResult> IdentityAsync(int onnxIndex, [AllowedFileType(new[] { ".jpg", ".jpeg", ".png", ".bmp" })] IFormFile file, string paramJson, bool adaptive = true)
        {
            ...
            if (result.GetDetails(out List<OnnxData>? datas))
            {
                ...
                IdentityOperate operate = IdentityOperate.Instance(new IdentityData
                {
                    ...
                    Hardware = new CoreMLExecutionProvider(Path.Combine(onnxData.path, onnxData.name), adaptive),
        ...
        /// </returns>
        [HttpPost]
        public async Task<OperateResult> IdentityDrawAsync(int onnxIndex, [AllowedFileType(new[] { ".jpg", ".jpeg", ".png", ".bmp" })] IFormFile file, string paramJson, bool adaptive = true)
        {
            OperateResult result = await QueryAsync(onnxIndex);
            if (result.GetDetails(out List<OnnxData>? datas))
            {
