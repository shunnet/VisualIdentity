...
namespace YoloDotNet.Core
{
    ...
    internal class YoloCore(YoloOptions yoloOptions) : IDisposable
    {
        ...
        public YoloOptions YoloOptions => yoloOptions;
        ...
        public void InitializeYolo()
        {
            ...
            if (OnnxModel.InputShapes.Count == 0)
                throw new YoloDotNetModelException("The model does not define an input tensor.");

            _inputShape = OnnxModel.InputShapes.First().Value;
            if (_inputShape.Length != 4 || _inputShape[0] != 1 || _inputShape[1] is not (1 or 3))
                throw new YoloDotNetModelException("The model input must have shape [1, 1|3, height, width].");
            ...
            var format = (_inputShape[1] == 1) ? SKColorType.Gray8 : SKColorType.Rgb888x;
        ...
        /// <param name="roi">An optional source region to preprocess.</param>
        ...
        /// <returns>An owned array containing non-overlapping bounding boxes ordered by confidence.</returns>
        ...
        public ObjectResult[] RemoveOverlappingBoxes(Span<ObjectResult> predictionSpan, double iouThreshold)
        {
            ...
            for (int i = 0; i < totalPredictions; i++)
            {
                ...
                {
                    if (CalculateIoU(item.BoundingBox, predictionSpan[j].BoundingBox) > iouThreshold)
                    {
                        overlapFound = true;
                        break;
                    }
                ...
                if (!overlapFound)
                    predictionSpan[counter++] = item;
            }

            return predictionSpan[..counter].ToArray();
        }
        ...
        {
            var results = new List<T>(detections.Length);
