...
namespace YoloDotNet.Modules.RTDETR
{
    internal class ObjectDetectionModuleRtdetr : IObjectDetectionModule
    {
        ...
        public ObjectDetectionModuleRtdetr(YoloCore yoloCore)
        {
            ...
            if (outputShape.Length != 3 || outputShape[1] <= 0 || outputShape[2] <= 4)
                throw new YoloDotNetModelException("RT-DETR output must have shape [batch, predictions, attributes].");
        ...
        private ObjectResult[] ObjectDetection(InferenceResult inferenceResult, double confidenceThreshold, double overlapThreshold)
        {
            ...
            try
            {
                var rowCount = Math.Min(_predictions, ortSpan.Length / _attributes);
                var classCount = Math.Min(_totalLabels, _attributes - _classOffset);
                for (int row = 0; row < rowCount; row++)
                {
                    var i = row * _attributes;
