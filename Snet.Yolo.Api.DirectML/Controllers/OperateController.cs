...
namespace Snet.Yolo.Api.Controllers
{
    ...
    public class OperateController : OperateBaseController
    {
        ...
        /// </returns>
        [HttpPost]
        public async Task<OperateResult> IdentityAsync(int onnxIndex, [AllowedFileType(new[] { ".jpg", ".jpeg", ".png", ".bmp" })] IFormFile file, string paramJson, int gpuid = 0)
        {
            ...
            if (result.GetDetails(out List<OnnxData>? datas))
            {
                ...
                IdentityOperate operate = IdentityOperate.Instance(new IdentityData
                {
                    ...
                    Hardware = new DirectMLExecutionProvider(Path.Combine(onnxData.path, onnxData.name), gpuid),
        ...
        /// </returns>
        [HttpPost]
        public async Task<OperateResult> IdentityDrawAsync(int onnxIndex, [AllowedFileType(new[] { ".jpg", ".jpeg", ".png", ".bmp" })] IFormFile file, string paramJson, int gpuid = 0)
        {
            OperateResult result = await QueryAsync(onnxIndex);
            if (result.GetDetails(out List<OnnxData>? datas))
            {
